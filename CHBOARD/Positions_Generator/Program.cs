...
namespace Positions_Generator
{
    class Program
    {
        private readonly ChevacaDB_Context _boardContext;
        private readonly ChapiDB_Context _apiContext;

        public Program(ChevacaDB_Context boardContext, ChapiDB_Context apiContext )
        {
        ...
        public void GMaps_GenerarRecorridoRandom(int duracion_segundos, int frecuencia_inserts_segundos, int cantidad_nodos, decimal posicion_inicial_lat, decimal posicion_inicial_long, int radio_dispersión_int, bool isLocal, bool isModoCompleto)
        {
                ...
                if (!string.IsNullOrWhiteSpace(connection_string_chapi) && !string.IsNullOrWhiteSpace(connection_string_chboard))
                {
                    ...
                    if (connection_chapi != null && connection_chboard != null)
                    {
                        ...
                        if (isModoCompleto)
                        {
                            ...
                            Person dueno = new Person();
                            dueno.Full_Name = "PERSONA TESTING";
                            dueno.Email = "1";
                            dueno.Identification = "1";
                            dueno.Date_born = DateTime.Now.AddYears(-50);
                            dueno.IsOwner = true;
                            ...
                            ranch.Employees.Add(dueno);
                            ranch.Ch_Organization_ID = 1;
                            ranch.Company_Name = "ESTANCIA TESTING";
                            ranch.Company_Rut = "1";
                            ranch.Company_SocialReason = "ESTANCIA TESTING";
                            ranch.Company_Domain = "ESTANCIA_TESTING";
                            ...
                            Land land = new Land();
                            land.Land_Name = "CAMPO TESTING";
                            land.Description = "1";
                            land.Chirpstack_App_ID = "1";
                            
                            ranch.Lands.Add(land);
                            ...
                            for (int i = 0; i < cantidad_nodos; i++) // Recorro cada nodo
                            {
                                ...
                                Ch_Device vaca_device = new Ch_Device();
                                vaca_device.Ch_Name = DEVICE_ID.ToString();
                                vaca_device.Ch_Dev_Eui = DEVICE_ID.ToString();
                                vaca_device.Ch_App_Key = DEVICE_ID.ToString();
                                vaca.Date_Born = DateTime.Now.AddMonths(-13);
                                vaca.Gender_MF = "F";
                                vaca.Code_Animal = 1;
                                vaca.List_animals_categories_code = 1;
                                vaca.List_animals_breeds_code = 1;
                                vaca.Ch_Device = vaca_device;
                        ...
                        while (DateTime.Now < hora_cierre && !boton_finalizar)
                        {
                            ...
                            for (int i = 0; i < cantidad_nodos; i++)
                            {
                                ...
                                if (new_lat >= -90 && new_lat <= 90 && new_long >= -180 && new_long <= 180)
                                {
                                    ...
                                    data.Datetime_Inicio = DateTime.Now;
                                    data.Datetime_Fin = DateTime.Now;

                                    _apiContext.payloads.Add(data);
